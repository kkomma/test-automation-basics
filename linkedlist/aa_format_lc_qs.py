a = ['1 Reverse Linked List: https://leetcode.com/problems/reverse-linked-list/description/',
...
for aa in a:
    q = aa.split(':')
    qs.append('_'.join(q[0].strip().split(' ')[1:]))

print(qs)

for f in qs:
    with open(f'/Users/veeranica/Desktop/pyt/coding/test-automation-basics/linkedlist/{f.lower()}.py', 'w') as file:
        file.write(f"write {f.lower().replace('_', ' ')} with main function")
